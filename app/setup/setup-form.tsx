...
export function SetupForm({ setupToken }: SetupFormProps) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!adminPassword || !personalPassword) {
      setError('Password setup could not be completed.')
      return
    }
    if (adminPassword === personalPassword) {
    ...
    return (
      <div className="rounded-2xl border border-white/15 bg-white/[0.04] p-6 text-center">
        ...
        <p className="mt-2 text-sm leading-6 text-white/60">
          The setup link is now permanently disabled.
        </p>
    ...
    <form onSubmit={submit} className="space-y-5">
      <div>
        ...
        <div className="relative">
          <input
            id="admin-password"
            className="w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 pr-12 text-sm text-white outline-none transition placeholder:text-white/25 focus:border-white/35"
      ...
      {error ? <p className="rounded-xl border border-white/15 bg-white/[0.03] px-4 py-3 text-sm text-white/75">{error}</p> : null}
      <button className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/[0.08] px-4 py-3 font-mono text-sm text-white transition hover:border-white/40 hover:bg-white/[0.12] disabled:cursor-not-allowed disabled:opacity-50" type="submit" disabled={pending}>
        {pending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}
        {pending ? 'Securing…' : 'Initialize passwords'}
      </button>
    </form>
  )
}
